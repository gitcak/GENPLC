...
static void ensureDefaultPreferences() {
    ...
    if (!pw.isKey("mHost")) pw.putString("mHost", "beam.soracom.io");
    if (!pw.isKey("mPort")) pw.putUInt("mPort", 1883);
    if (!pw.isKey("mUser")) pw.putString("mUser", "k-0000");
    if (!pw.isKey("mPass")) pw.putString("mPass", "HbC8wO+i1/hl4ogngeWABA==");
    pw.end();
}
...
bool settingsLoad(AppSettings& s){
    ...
    if(!p.begin(NS, true)) {
        ...
        strlcpy(s.mqttHost, "beam.soracom.io", sizeof(s.mqttHost));
        s.mqttPort = 1883;
        strlcpy(s.mqttUser, "k-0000", sizeof(s.mqttUser));
        strlcpy(s.mqttPass, "HbC8wO+i1/hl4ogngeWABA==", sizeof(s.mqttPass));
        return true;
    }
